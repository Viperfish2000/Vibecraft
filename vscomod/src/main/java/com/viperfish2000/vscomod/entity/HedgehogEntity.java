...
public class HedgehogEntity extends TameableEntity {
	   private static final Ingredient TEMPTATION_ITEMS = Ingredient.fromItems(Items.SWEET_BERRIES);
	private static final DataParameter<Float> DATA_HEALTH_ID = EntityDataManager.createKey(HedgehogEntity.class, DataSerializers.FLOAT);
   private static final DataParameter<Boolean> BEGGING = EntityDataManager.createKey(HedgehogEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Integer> COLLAR_COLOR = EntityDataManager.createKey(HedgehogEntity.class, DataSerializers.VARINT);
   ...
   public HedgehogEntity(EntityType<? extends HedgehogEntity> type, World worldIn) {
      super(type, worldIn);
      this.setTamed(false);
   }
   ...
   //   this.goalSelector.addGoal(3, new HedgehogEntity.AvoidEntityGoal(this, LlamaEntity.class, 24.0F, 1.5D, 1.5D));
  ...
  //    this.targetSelector.addGoal(5, new NearestAttackableTargetGoal<>(this, AbstractSkeletonEntity.class, false));
   ...
   protected SoundEvent getAmbientSound() {
      if (this.isAngry()) {
         ...
         return this.isTamed() && this.dataManager.get(DATA_HEALTH_ID) < 10.0F ? SoundMod.ENTITY_HEDGEHOG_WHINE : null;
      } else {
         return null;
      }
   }
   protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
      return SoundMod.ENTITY_HEDGEHOG_HURT;
   }

   protected SoundEvent getDeathSound() {
      return SoundMod.ENTITY_HEDGEHOG_DEATH;
   }
...
**/
   ...
   public HedgehogEntity createChild(AgeableEntity ageable) {
      HedgehogEntity HedgehogEntity = EntityRegistry.HEDGEHOG.create(this.world);
      UUID uuid = this.getOwnerId();
      if (uuid != null) {
         HedgehogEntity.setOwnerId(uuid);
   ...
   public boolean canMateWith(AnimalEntity otherAnimal) {
      if (otherAnimal == this) {
         ...
         return false;
      } else if (!(otherAnimal instanceof HedgehogEntity)) {
         return false;
      } else {
         HedgehogEntity HedgehogEntity = (HedgehogEntity)otherAnimal;
         if (!HedgehogEntity.isTamed()) {
            return false;
         } else if (HedgehogEntity.isSitting()) {
            return false;
         } else {
            return this.isInLove() && HedgehogEntity.isInLove();
         }
      }
   }
   ...
   public boolean shouldAttackEntity(LivingEntity target, LivingEntity owner) {
      if (!(target instanceof CreeperEntity) && !(target instanceof GhastEntity)) {
         if (target instanceof HedgehogEntity) {
            HedgehogEntity HedgehogEntity = (HedgehogEntity)target;
            if (HedgehogEntity.isTamed() && HedgehogEntity.getOwner() == owner) {
               return false;
            }
         }
   ...
   class AvoidEntityGoal<T extends LivingEntity> extends net.minecraft.entity.ai.goal.AvoidEntityGoal<T> {
      private final HedgehogEntity wolf;

      public AvoidEntityGoal(HedgehogEntity wolfIn, Class<T> p_i47251_3_, float p_i47251_4_, double p_i47251_5_, double p_i47251_7_) {
      ...
      private boolean avoidLlama(LlamaEntity p_190854_1_) {
         return p_190854_1_.getStrength() >= HedgehogEntity.this.rand.nextInt(5);
      }
      ...
      public void startExecuting() {
         HedgehogEntity.this.setAttackTarget((LivingEntity)null);
